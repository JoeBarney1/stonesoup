...
class ParticleSmoother(Smoother):
    ...
    def particle_paths(self,track=None, max_lag=None):
        ...
        for t in range(track_length):
            ...
            # shape (num_particles,)
            idx_t = particle_indices[:, t].astype(int)
            ...
            # Vector-index the state vector and covariance all at once:
            # Each is shape (M, M, num_particles) or (M, num_particles).
            # The '...,' means 'take all preceding dims'
            new_state_vector = state_t.state_vector[..., idx_t]   # shape (M, num_particles)
            new_covar = state_t.covariance[..., idx_t]            # shape (M, M, num_particles)

            # Do the same for the prediction side
            pred_state_vector = prediction.state_vector[..., prev_idx_t]
            pred_covar = prediction.covariance[..., prev_idx_t]

            # Build a single MarginalisedParticleState for time t
            # (or MarginalisedParticleStateUpdate if you have a measurement)
            particle_prediction = MarginalisedParticleStatePrediction(
                state_vector=pred_state_vector,
                covariance=pred_covar,
    ...
    #TODO: put the max lag stuff inside the t loop so that we only loop for range(num_timesteps),
    # But with each time 't' being capped by min(t+max lag , track_length-1) and then generating idx from there,
