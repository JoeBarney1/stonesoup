...
def test_phd_single_component_update(UpdaterClass, measurement_model,
    ...
    assert np.allclose(updated_component.mean, eval_posterior.mean, 0, atol=1.e-14)
    assert np.allclose(updated_component.covar, eval_posterior.covar, 0, atol=1.e-14)
    ...
    assert np.allclose(miss_detected_component.mean, prediction.mean, 0, atol=1.e-14)
    assert np.allclose(miss_detected_component.covar, prediction.covar, 0, atol=1.e-14)
