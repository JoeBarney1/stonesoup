...
class MarginalisedParticleState(ParticleState):
    """Marginalised particle state type

    This is a particle state object which describes the state as a
    distribution of particles, used by the marginalised particle filter.
    """

    # need to do dimensionality check on this covariance, need it to be 3-d
    covariance: np.ndarray = Property(
        doc="Three dimensional array (m, m, n) of covariance matrices"
    )
    ...
    def covar(self):
        ...
        weighted_covar = np.sum(
            self.weight[np.newaxis, np.newaxis, :] * covariance,
            axis=len(covariance.shape) - 1,
        )

        mu_bar = np.sum(self.weight[np.newaxis, :] * mu, axis=1)
        tmp = mu - mu_bar[:, np.newaxis]
        weighted_mean = np.sum(
            self.weight[np.newaxis, np.newaxis, :] * (np.einsum("ik,jk->ijk", tmp, tmp)),
            axis=len(covariance.shape) - 1,
        )
        return np.array(weighted_mean + weighted_covar, dtype=float)
    ...
    @clearable_cached_property("state_vector", "log_weight", "covariance")
    def mean(self):
        """Sample mean for particles"""
        if len(self) == 1:  # No need to calculate mean
            return self.state_vector
...
State.register(MarginalisedParticleState)
